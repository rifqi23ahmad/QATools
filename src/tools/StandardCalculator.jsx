...
function StandardCalculator() {
  ...
  // --- PERUBAHAN: Menambahkan tombol 'backspace' agar grid 4 kolom ---
  const buttons = [
    { id: 'clear', label: 'AC', class: styles.op, action: clearAll },
    { id: 'backspace', label: '⌫', class: styles.op, action: handleBackspace }, // <-- TOMBOL BARU
    { id: 'percent', label: '%', class: styles.op, action: handlePercent },
    { id: 'divide', label: '÷', class: styles.op, action: () => performOperation('/') },

    { id: 'seven', label: '7', action: () => inputDigit(7) },
    { id: 'eight', label: '8', action: () => inputDigit(8) },
    { id: 'nine', label: '9', action: () => inputDigit(9) },
    { id: 'multiply', label: '×', class: styles.op, action: () => performOperation('*') },

    { id: 'four', label: '4', action: () => inputDigit(4) },
    { id: 'five', label: '5', action: () => inputDigit(5) },
    { id: 'six', label: '6', action: () => inputDigit(6) },
    { id: 'subtract', label: '−', class: styles.op, action: () => performOperation('-') },

    { id: 'one', label: '1', action: () => inputDigit(1) },
    { id: 'two', label: '2', action: () => inputDigit(2) },
    { id: 'three', label: '3', action: () => inputDigit(3) },
    { id: 'add', label: '+', class: styles.op, action: () => performOperation('+') },

    { id: 'zero', label: '0', class: styles.zero, action: () => inputDigit(0) },
    { id: 'decimal', label: '.', action: inputDecimal },
    { id: 'equals', label: '=', class: styles.op, action: handleEquals },
  ];
  // --- AKHIR PERUBAHAN ---

  return (
    <div className={styles.standardCalculator} tabIndex="0" onKeyDown={handleKeyDown}>
      <div className={styles.calcDisplay}>
        {display}
      </div>

      <div className={styles.standardCalculatorGrid}>
        {buttons.map((b) => (
          <button
            key={b.id}
            id={b.id}
            className={`${styles.calcButton} ${b.class || ''}`} // Menggunakan b.class
            onClick={b.action}
            aria-label={b.label}
          >
            {b.label}
