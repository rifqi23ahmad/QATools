...
function TimesheetTracker() {
  ...
  const [loading, setLoading] = useState(!!localStorage.getItem(LS_USER_CODE));
  ...
  const [settingHour, setSettingHour] = useState(() => {
      const h = localStorage.getItem(LS_DEADLINE_HOUR);
      return h ? parseInt(h) : DEFAULT_HOUR;
  });
  
  const [settingMinute, setSettingMinute] = useState(() => {
      const m = localStorage.getItem(LS_DEADLINE_MINUTE);
      return m ? parseInt(m) : DEFAULT_MINUTE;
  });
  
  const [settingDays, setSettingDays] = useState(() => {
      const d = localStorage.getItem(LS_ALARM_DAYS);
      try { return d ? JSON.parse(d) : DEFAULT_DAYS; } catch { return DEFAULT_DAYS; }
  });
  ...
  const handleRequestDelete = (id) => { setIdToDelete(id); };
  ...
  const cancelDeleteAction = () => { setIdToDelete(null); };

  const filteredEntries = useMemo(() => {
      return entries.filter(e => e.work_date.startsWith(filterMonth));
  }, [entries, filterMonth]);

  const summary = useMemo(() => {
      const totalHours = filteredEntries.reduce((acc, curr) => acc + (parseFloat(curr.duration) || 0), 0);
  ...
  const isTodayActive = settingDays.includes(todayIndex);
  ...
  return (
    <div className={styles.container}>
      ...
      {/* BANNER */}
      {loading ? (
         <div className={styles.reminderBanner} style={{backgroundColor: '#f8fafc', borderColor: '#e2e8f0', color: '#64748b', animation: 'none', display: 'flex', alignItems: 'center', gap: '1rem'}}>
             <div className="loader-spinner" style={{width: '20px', height: '20px', border: '2px solid #cbd5e0', borderTopColor: '#475569'}}></div>
             <span>Memeriksa status...</span>
         </div>
      ) : (
         !hasFilledToday && !isTodayActive ? (
            <div className={styles.reminderBanner} style={{backgroundColor: '#f1f5f9', borderColor: '#cbd5e0', color: '#64748b', animation: 'none'}}>
                <i className="fas fa-mug-hot fa-2x"></i>
                <div><strong>Hari Libur!</strong><div>Anda tidak diwajibkan mengisi timesheet hari ini.</div></div>
            </div>
         ) : (
            <div className={`${styles.reminderBanner} ${hasFilledToday ? styles.success : ''}`}>
                {hasFilledToday ? (
                    <><i className="fas fa-check-circle fa-2x"></i><div><strong>Aman!</strong> Timesheet hari ini sudah terisi.</div></>
                ) : (
                    <><i className="fas fa-clock fa-2x"></i><div><strong>Belum Isi!</strong> Deadline: {deadlineStr}. {parseInt(currentTimeStr.split(':')[0]) >= settingHour ? " (Telat!)" : ""}</div></>
                )}
            </div>
         )
      )}
      ...
      {idToDelete && (
        <div className={styles.modalOverlay}>
            <div className={styles.deleteCard}>
                <div className={styles.deleteIconWrapper}><i className="fas fa-trash-alt"></i></div>
                <h3 className={styles.deleteTitle}>Hapus Entri?</h3>
                <p className={styles.deleteMessage}>Apakah Anda yakin ingin menghapus aktivitas ini? Tindakan ini tidak dapat dibatalkan.</p>
      ...
      {showSettings && (
          <div className={styles.modalOverlay}>
              <div className={styles.modalContent}>
                  <div className={styles.modalHeader}><h3>⏰ Pengaturan Alarm</h3><button onClick={() => setShowSettings(false)} className={styles.closeBtn}>&times;</button></div>
                  <div className={styles.modalBody}>
                      ...
                      <div className={styles.daySelector}>
                          {DAYS_LABEL.map((d) => (
                              <button key={d.id} className={`${styles.dayBtn} ${settingDays.includes(d.id) ? styles.dayBtnActive : ''}`} onClick={() => toggleDay(d.id)}>{d.label}</button>
                          ))}
                      </div>
                  </div>
                  <div className={styles.modalFooter}><button onClick={saveSettings} className="button primary" style={{width: '100%'}}>Simpan Pengaturan</button></div>
              </div>
          </div>
      )}
      ...
      <div className={`${styles.formCard} ${editingId ? styles.editingMode : ''}`}>
        <div className={styles.formHeader}>
            <h3 className="label" style={{margin:0, fontSize: '1.1rem'}}>{editingId ? '✏️ Edit Aktivitas' : '📝 Tambah Aktivitas Baru'}</h3>
        ...
        <form onSubmit={handleSubmit} className={styles.formGrid}>
          <div className={styles.rowTwoColumns}>
            <div className={styles.formGroup}>
              ...
              <CustomDatePicker value={inputDate} onChange={setInputDate} max={getTodayStr()} />
            </div>
            <div className={styles.formGroup}>
      ...
      <div className={styles.historySection}>
        <div className={styles.historyHeader}>
            <div className={styles.filterGroup}>
                <label style={{fontSize:'0.9rem', fontWeight:'600', color:'#4a5568'}}>Filter Bulan:</label>
                {/* DISINI KITA GANTI JADI CUSTOM PICKER MODE MONTH */}
                <div style={{ width: '180px' }}>
                    <CustomDatePicker 
                        value={filterMonth + '-01'} // Trik agar formatnya YYYY-MM-DD
                        onChange={(val) => setFilterMonth(val.slice(0, 7))} // Ambil YYYY-MM saja
                        max={getCurrentMonthStr() + '-01'}
                        pickerType="month"
                    />
                </div>
            </div>
            <div className={styles.monthSummary}>
                <div className={styles.summaryItem}><i className="fas fa-clock text-blue-500"></i><span>Total Jam: <span className={styles.summaryValue}>{summary.totalHours}</span></span></div>
                <div className={styles.summaryItem}><i className="fas fa-calendar-day text-green-500"></i><span>Hari Kerja: <span className={styles.summaryValue}>{summary.uniqueDays}</span></span></div>
            </div>
        </div>
        <div className={styles.tableScrollWrapper}>
            <table className={styles.historyTable}>
            <thead className={styles.stickyHeader}>
                <tr><th style={{width: '130px'}}>Tanggal</th><th>Aktivitas</th><th style={{textAlign:'right', width: '100px'}}>Aksi</th></tr>
            </thead>
            <tbody>
                {loading ? (
                    <tr><td colSpan="3" style={{textAlign:'center', padding:'2rem', color:'#999'}}>Memuat data...</td></tr>
                ) : filteredEntries.length === 0 ? (
                    <tr><td colSpan="3" style={{textAlign:'center', padding:'2rem', color:'#999'}}>Tidak ada data di bulan ini.</td></tr>
                ) : (
                    filteredEntries.map(e => (
                        <tr key={e.id} className={editingId === e.id ? styles.rowEditing : ''}>
                            <td style={{verticalAlign: 'top'}}><span className={styles.dateBadge}>{formatDateToID(e.work_date)}</span>{e.duration > 0 && <div style={{fontSize:'0.8rem', color:'#718096', marginTop:'4px'}}>{e.duration} Jam</div>}</td>
                            ...
                            <td style={{textAlign:'right', verticalAlign: 'top'}}>
                                <div style={{display:'flex', justifyContent:'flex-end', gap:'6px'}}>
                                    ...
                                    <button className="button secondary" onClick={()=>handleRequestDelete(e.id)} title="Hapus" style={{padding:'6px 10px', color:'#e53e3e', borderColor:'#fed7d7', background:'#fff5f5'}}><i className="fas fa-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                    ))
