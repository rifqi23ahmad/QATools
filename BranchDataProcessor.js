function initBranchDataProcessor() {
    const page = document.getElementById('BranchDataProcessor');
    // HTML diperbarui untuk menggunakan <label> yang di-styling sebagai tombol
    page.innerHTML = `
        ...
        <div class="card">
            <div class="grid grid-cols-1 md-grid-cols-3 gap-4 items-center bg-gray-50 p-4 rounded-lg mb-6 border">
                <div>
                    <input type="file" id="excel-file-input" class="is-hidden" accept=".xlsx, .xls">
                    <label for="excel-file-input" class="button secondary" style="width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: inline-block;">
                        <i class="fas fa-upload" style="margin-right: 0.5rem;"></i>
                        <span>Pilih File Excel...</span>
                    </label>
                </div>
                 <button id="process-btn" class="button primary" disabled>
                    Proses Data
                </button>
                <button id="download-btn" class="button success" disabled>
                    Unduh Hasil (.xlsx)
                </button>
            </div>
            <p id="file-info" class="text-center text-gray-500 text-sm mb-6 h-5"></p>


            <div id="table-area" class="is-hidden grid grid-cols-1 lg-grid-cols-2 gap-8">
                <div>
                    <h2 class="text-xl font-semibold mb-3 text-gray-600">Pratinjau Data Asli</h2>
                    <div class="table-container results-table-wrapper border rounded-lg">
                        <table class="results-table">
                            <thead id="original-data-head" class="bg-gray-50 sticky top-0"></thead>
                            <tbody id="original-data-body" class="bg-white divide-y divide-gray-200"></tbody>
                        </table>
                    </div>
                </div>
                <div>
                    <h2 class="text-xl font-semibold mb-3 text-green-600">Pratinjau Hasil Proses</h2>
                    <div class="table-container results-table-wrapper border rounded-lg">
                        <table class="results-table">
                            <thead id="processed-data-head" class="bg-green-50 sticky top-0"></thead>
                            <tbody id="processed-data-body" class="bg-white divide-y divide-gray-200"></tbody>
                        </table>
                    </div>
                </div>
            </div>
    ...
    // Logika baru untuk mengupdate teks pada label tombol
    const fileInputLabel = document.querySelector('label[for="excel-file-input"]');
    const fileInputLabelSpan = fileInputLabel.querySelector('span');
    const originalLabelText = fileInputLabelSpan.textContent;

    // Mapping data cabang (as provided in your script)
    ...
    fileInput.addEventListener('change', (event) => {
        ...
        if (uploadedFile) {
            fileInfo.textContent = `File dipilih: ${uploadedFile.name}`;
            fileInputLabelSpan.textContent = uploadedFile.name; // Update the label text
            processBtn.disabled = false;
            downloadBtn.disabled = true;
            tableArea.classList.add('is-hidden');
        } else {
            fileInfo.textContent = '';
            fileInputLabelSpan.textContent = originalLabelText; // Reset the label text
    ...
    processBtn.addEventListener('click', () => {
        ...
        fileInfo.textContent = 'Membaca dan memproses file, harap tunggu...';
        ...
        reader.onload = function(event) {
            try {
                ...
                // Mengubah worksheet menjadi array of objects
                ...
                const previewOriginalData = originalData.slice(0, PREVIEW_ROWS);
                const previewProcessedData = processedData.slice(0, PREVIEW_ROWS);

                renderTable('original-data-head', 'original-data-body', previewOriginalData, originalHeaders);
                renderTable('processed-data-head', 'processed-data-body', previewProcessedData, originalHeaders);
                
                tableArea.classList.remove('is-hidden');
                downloadBtn.disabled = false;
                
                if (originalData.length > PREVIEW_ROWS) {
                    fileInfo.textContent = `Berhasil! Menampilkan pratinjau ${PREVIEW_ROWS} dari ${originalData.length} total baris.`
                } else {
                     fileInfo.textContent = `${originalData.length} baris data berhasil diproses.`
                }
    ...
    downloadBtn.addEventListener('click', () => {
        ...
        // 1. Buat worksheet baru dari data yang diproses
        const newWorksheet = XLSX.utils.json_to_sheet(processedData);
        
        // 2. Buat workbook baru
        const newWorkbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(newWorkbook, newWorksheet, 'Hasil Proses');

        // 3. Buat file Excel untuk diunduh
        const originalFilename = uploadedFile.name.replace(/\.(xlsx|xls)$/i, '');
        XLSX.writeFile(newWorkbook, `${originalFilename}_processed.xlsx`);
    });

    // --- Functions ---
    
    function processData(data) {
        // [NEW] Buat pemetaan terbalik untuk pencarian code-to-name yang efisien.
        ...
        processed.forEach(row => {
             // Fungsi bantuan untuk memeriksa nilai kosong/null
            const isEmpty = (val) => val === null || val === undefined || String(val).trim() === '';

            // --- PRE-PROCESSING: Standarisasi nama kolom untuk akses yang andal ---
            const cleanRow = {};
            ...
            // LANGKAH 1: Lengkapi informasi Cabang (saling mengisi)
            ...
            // Perbarui cleanRow untuk langkah-langkah selanjutnya dalam loop ini
            for (const key in row) {
                cleanRow[key.trim().toLowerCase()] = row[key];
            }

            // LANGKAH 2: Lengkapi informasi Lokasi Unit (saling mengisi)
            ...
            // LANGKAH 3: Aturan fallback jika info Lokasi Unit KOSONG TOTAL
            ...
            // LANGKAH 4: Koreksi Final - pastikan unit_location_code selalu sinkron dengan unit_location_name
    ...
    function renderTable(headId, bodyId, data, headers) {
        ...
        if (!data || data.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="${headers.length || 5}" class="text-center py-4">Tidak ada data untuk ditampilkan.</td></tr>`;
            return;
        }

        let headerHTML = '<tr>';
        headers.forEach(header => {
            headerHTML += `<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">${header.replace(/_/g, ' ')}</th>`;
        });
        headerHTML += '</tr>';
        ...
        data.forEach(item => {
            ...
            headers.forEach(header => {
                ...
                const displayValue = value.toString().trim() === '' ? '<span class="text-red-500 font-semibold">KOSONG</span>' : value;
                rowHTML += `<td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">${displayValue}</td>`;
            });
